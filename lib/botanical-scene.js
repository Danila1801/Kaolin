...
(function () {
  ...
  var MIST = [0.902, 0.941, 0.914];   // #E6F0E9 fog tint
  ...
  var BG_FS = '#version 300 es\n' +
    ...
    'uniform vec3 uBg, uSand, uForest, uMist;\n' +
    'out vec4 frag;\n' +
    'float hash12(vec2 p){vec3 p3=fract(vec3(p.xyx)*.1031);p3+=dot(p3,p3.yzx+33.33);return fract((p3.x+p3.y)*p3.z);}\n' +
    'float vnoise(vec2 p){vec2 i=floor(p),f=fract(p);f=f*f*(3.-2.*f);\n' +
    ' float a=hash12(i),b=hash12(i+vec2(1.,0.)),c=hash12(i+vec2(0.,1.)),d=hash12(i+vec2(1.,1.));\n' +
    ...
    // drifting fog: two octaves of value noise in a horizon band. Mint mist in
    ...
    '  c = mix(c, uMist, band * fbm * 0.60 * (1.0 - uDark));\n' +
  ...
  class BotanicalScene extends HTMLElement {
    ...
    connectedCallback() {
      ...
      ['uRes', 'uDark', 'uGrain', 'uTime', 'uBg', 'uSand', 'uForest', 'uMist'].forEach((n) => {
    ...
    _frame(now) {
      ...
      gl.uniform3fv(this._uni.g_uMist, MIST);
