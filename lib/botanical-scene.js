...
(function () {
  ...
  var FOG = [0.831, 0.863, 0.788];    // #D4DCC9 warm-sage daylight fog (was cool mint #E6F0E9)
  ...
  var BLADE_VS = '#version 300 es\n' +
    ...
    '  vFog = smoothstep(uSpan*0.36, uSpan*0.85, rel);\n' + // depth melts into the mist a little sooner
  ...
  var BLADE_FS = '#version 300 es\n' +
    ...
    '  c = mix(c, cd, uDark);\n' +
    '  vec3 fog = mix(uFog, uBg, uDark);\n' + // warm sage by day; night keeps uBg (faint green)
    '  c = mix(c, fog, vFog);\n' +
    '  frag = vec4(c, 1.0);\n' +
    '}\n';
  ...
  class BotanicalScene extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this._uni = {};
      ['uVP', 'uTime', 'uWind', 'uCamZ', 'uSpan', 'uMouse', 'uDark', 'uBg', 'uSage', 'uForest', 'uSand', 'uFog'].forEach((n) => {
        this._uni['b_' + n] = gl.getUniformLocation(this._pBlade, n);
      });
      ['uRes', 'uDark', 'uGrain', 'uTime', 'uBg', 'uSand', 'uForest', 'uFog'].forEach((n) => {
      ...
      // One-time buffer realloc before the second frame: some compositors
      // (Intel/ANGLE) otherwise keep displaying the canvas's initial blank
      // frame until a scroll invalidates the layer. A fresh drawing buffer
      // forces a fresh compositor resource.
      var kicked = 0;
      var loop = (now) => {
        this._raf = requestAnimationFrame(loop);
        if (kicked === 1) {
          this._resize();
          // …and nudge the scroll offset once (1px, restored next frame):
          // a real scroll invalidation is the only signal some compositor
          // builds accept before they start pulling fresh canvas frames.
          if (window.scrollY === 0) {
    ...
    _frame(now) {
      ...
      gl.uniform3fv(this._uni.g_uFog, FOG);
      gl.uniform1f(this._uni.g_uTime, this._time);
      ...
      gl.uniform3fv(this._uni.b_uFog, FOG);
